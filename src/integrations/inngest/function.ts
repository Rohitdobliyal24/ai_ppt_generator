...
// import { generateSlideImage } from '#/lib/generate-image'
//Open-AI 
// ---------------------------------------------------------------------------
// Image Generation
// ---------------------------------------------------------------------------
...
// }
function buildRandomImageKitUrl(_: string, i: string): string {
  return `https://picsum.photos/1280/720?random=${i}`
}
...
`
  ...
  slides.map(async (s, i) => {
    // const imageUrl = await generateSlideImage(s.imagePrompt)
    return {
      presentationId,
      order: i,
      title: s.title,
      ...
      imageUrl: buildRandomImageKitUrl(s.imagePrompt, `${presentationId}-${i}`),
    }
  })
)
